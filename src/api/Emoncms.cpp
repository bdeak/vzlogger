/***********************************************************************/
/** @file Emoncms.cpp
 * Header file for emnocms.org API calls
 *
 * @author Steffen Vogel <info@steffenvogel.de>
 * @copyright Copyright (c) 2011, The emnocms.org project
 * @package vzlogger
 ...
 * along with emnocms.org. If not, see <http://www.gnu.org/licenses/>.
 */
	...
	, _lastReadingSent (0)
{
	...
	try {
		...
		print(log_error, "api emnocms requires parameter \"middleware\" but it's missing!", ch->name());
		throw;
	} catch (vz::VZException &e) {
		print(log_error, "api emnocms requires parameter \"middleware\" as string but seems to have different type!", ch->name());
		throw;
	}
	...
	try {
		...
		print(log_error, "api emnocms requires parameter \"timeout\" as integer but seems to have different type!", ch->name());
		throw;
	}

	try {
		_node = optlist.lookup_int(pOptions, "node");
	} catch (vz::OptionNotFoundException &e) {
		print(log_error, "api emnocms requires parameter \"node\" but it's missing!", ch->name());
		throw;
	} catch (vz::VZException &e) {
		print(log_error, "api emnocms requires parameter \"node\" as string but seems to have different type!", ch->name());
		throw;
	}

	try {
		_apikey = optlist.lookup_int(pOptions, "apikey");
	} catch (vz::OptionNotFoundException &e) {
		print(log_error, "api emnocms requires parameter \"apikey\" but it's missing!", ch->name());
		throw;
	} catch (vz::VZException &e) {
		print(log_error, "api emnocms requires parameter \"apikey\" as string but seems to have different type!", ch->name());
		throw;
	}
	...
	_api.headers = curl_slist_append(_api.headers, _auth_header.append(_apikey).c_str());
...
void vz::api::Emoncms::send()
{
	CURLresponse response;
	json_object *json_objs = json_object_new_array();
  ...
  std::string query_string;
	...
	json_objs = api_json_tuples(channel()->buffer());
  for (int i=0; i<json_object_array_length(json_objs);++i) {
    query_string = "";
    json_obj = json_object_array_get_idx(json_objs, i);
    int64_t timestamp = json_object_get_int64(json_object_array_get_idx(json_obj, 0));
    double value = json_object_get_double(json_object_array_get_idx(json_obj, 1));
    ...
    query_string = "time=";
    query_string.append(std::to_string(timestamp));
    query_string.append("&node=");
    query_string.append(_node);
    query_string.append("&csv=");
    query_string.append(std::to_string(value));
    
    curl_easy_setopt(_api.curl, CURLOPT_URL, _url.append(query_string).c_str());
  	...
  	print(log_debug, "JSON request: %s %s%s", channel()->name(), _url, query_string);
...
void vz::api::Emoncms::register_device() {
}


json_object * vz::api::Emoncms::api_json_tuples(Buffer::Ptr buf) {
...
void vz::api::Emoncms::api_parse_exception(CURLresponse response, char *err, size_t n) {
